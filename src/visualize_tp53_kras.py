...
def visualize_top_genes(mutexc_dir, top_gene):
    """
    Create a network diagram for a top gene based on the number of cohorts 
    it has mutually exclusive mutations in with other genes
    """
    ...
    for node in G.nodes():
        if node == top_gene: # The top gene gets its own neutral color
